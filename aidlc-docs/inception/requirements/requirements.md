...
| FR-09-05 | Gradle カスタムタスク（`injectIoTConfig`・`injectCertificates`）で IoT エンドポイント・X.509 証明書を Pixel Watch ビルドに自動注入する。CDK は証明書を `cdk/output/certs/` に出力し、Gradle タスクが `watch/app/src/main/res/raw/` へコピーする | 必須 |
...
| 時刻 | イベント | 対応要件 |
|------|---------|---------|
| **T0** | **Pixel Watch が SLEEP/ANGER のトリガーとなる生体データを送信した瞬間**（← レイテンシ計測の基準点。実際のパターン判定は BiometricAnalyzerLambda が T0+<1s 以降に実施） | — |
...
| NFR ID | 要件 | タイムライン基準 |
|--------|------|----------------|
| NFR-01-01 | SLEEP/ANGER → 音声開始まで 5秒以内 | T0（Pixel Watch データ送信）→ T0+<5s |
| NFR-01-02 | 生体トリガー → ブラウザ更新まで 2秒以内 | T0（Pixel Watch データ送信）→ T0+<2s |
| NFR-01-03 | LATE → SMS 送信まで 10秒以内 | T0（EventBridge 起動）→ T0+<10s |
| NFR-01-04 | MQTT センサーデータのクラウド到達: サブ秒 | T0（Pixel Watch データ送信）→ T0+<500ms |
