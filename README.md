...
- **課題**: EyeSight（スバルのステレオカメラによる衝突警告・自動ブレーキ）・DMS（ドライバーモニタリングシステム：カメラで顔・視線を検知し眠気や脇見を警告）・Autopilot（Teslaの車速・操舵を自動制御する運転支援機能）など既存の運転安全技術は「警告」で終わる。警告に気づき、自制するのはドライバー自身の負担
