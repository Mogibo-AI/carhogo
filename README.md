...
ドライバーの「自制心」を完全に奪い去る、過保護なAIアシスタント

## 💡 プロジェクト概要

居眠り・怒り・遅刻ストレスという運転中の3大リスクを、生体信号からリアルタイムで検知し、ドライバーが自覚する前にシステムが介入・解消します。

- **課題**: EyeSight・DMS・Autopilot など既存の運転安全技術は「警告」で終わる。警告に気づき、自制するのはドライバー自身の負担
...
> **既存技術の補足**:
> - **EyeSight**（スバル）: ステレオカメラによる衝突警告・自動ブレーキ
> - **DMS**: ドライバーモニタリングシステム — カメラで顔・視線を検知し眠気や脇見を警告
> - **Autopilot**（Tesla）: 車速・操舵を自動制御する運転支援機能

> 競合システムとの詳細な差別化分析は [要件定義書](aidlc-docs/inception/requirements/requirements.md) を参照。
...
| LATE検知 → SMS送信完了 | 10秒以内（※ベストエフォート） |
| システム稼働率 | 99.9%以上 |

> ※ LATE の「10秒以内」は Nova 2 Sonic の読み上げ時間とドライバー応答時間を含むため、会話が長引いた場合は超過する可能性があります。詳細は [要件定義書](aidlc-docs/inception/requirements/requirements.md) の「イベントタイムライン定義」を参照。
